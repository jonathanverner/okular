...
: QFrame( parent, isReply ? Qt::Widget | Qt::FramelessWindowHint : Qt::SubWindow ), m_annot( annot ), m_document( document ), m_page( page ), m_reply( isReply )
{
    ...
    middlelay->addItem( new QSpacerItem( 5, 5, QSizePolicy::Expanding, QSizePolicy::Fixed ) );
    QPushButton *replyBtn = new QPushButton(this);
    replyBtn->setText( i18n("Reply") );
    middlelay->addWidget( replyBtn );
    connect(replyBtn, SIGNAL(clicked(bool)),
            this, SLOT(slotReply()) );
...
void AnnotWindow::slotReply()
{
    Okular::TextAnnotation *annotation = new Okular::TextAnnotation();
    annotation->setBoundingRectangle(m_annot->boundingRectangle());
    ...
    annotation->window().setSummary("Comment");
    m_document->addPageAnnotation( m_page, annotation );
    annotation->makeRevisionOf( m_annot );
    mainlay->insertLayout(3,replyLayout(annotation));

    qDebug()<<"Created new reply to: " << annotation->revisionOf();
